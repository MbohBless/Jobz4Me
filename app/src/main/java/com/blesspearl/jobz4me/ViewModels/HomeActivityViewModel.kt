...
class HomeActivityViewModel : ViewModel() {
    ...
    var searchList: MutableLiveData<List<Course>>? = null
            ...
            udemyApiInterface.enqueue(object : Callback<Courses> {
                override fun onResponse(call: Call<Courses>, response: Response<Courses>) {
                    if (response.isSuccessful) {
                        ...
                        Toast.makeText(context, "${response.code()}hello", Toast.LENGTH_SHORT)
                            .show()
                    }
                }
 ...
 fun getSearch(search: String): LiveData<List<Course>> {
            ...
            udemyApiInterface.enqueue(object : Callback<Courses> {
                override fun onResponse(call: Call<Courses>, response: Response<Courses>) {
                    if (response.isSuccessful) {
                        Log.d("Works", "onResponse:${response.body()} ")
                        val courses: Courses = response.body()!!
                        courseList.addAll(courses.results)
                        searchList!!.value = courseList
                    }
                }
                override fun onFailure(call: Call<Courses>, t: Throwable) {
                }
        ...
        return searchList as MutableLiveData<List<Course>>
    }
}